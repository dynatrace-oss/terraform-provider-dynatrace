...
| Resource name | API endpoint | API token / OAuth permissions |
| ------------- | ----------- | ---------------------------- |
...
| dynatrace_automation_k8s_connections | /api/v2/settings/objects (schema: app:dynatrace.kubernetes.connector:connection) | settings.read, settings.write |
...
| dynatrace_database_app_feature_flags | /api/v2/settings/objects (schema: app:dynatrace.database.overview:feature-flags) | settings.read, settings.write |
