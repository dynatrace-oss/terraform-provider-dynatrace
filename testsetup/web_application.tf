locals {
  web_application_name = "Web Application"
}
data "dynatrace_application" "setup_application" {
  name = local.web_application_name
}

import {
  to       = dynatrace_web_application.application
  for_each = try(data.dynatrace_application.setup_application.id, null) == null ? [] : [data.dynatrace_application.setup_application.id]
  id       = each.value
}
