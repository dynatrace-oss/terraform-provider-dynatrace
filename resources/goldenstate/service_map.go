...
var SingleConfigServiceMap = map[export.ResourceType]ServiceFunc{
	...
	// For every configured Browser Monitor there automatically exists one of these records
	// Recommendation is to handle "golden state" via Browser Monitors
	export.ResourceTypes.BrowserMonitorPerformanceThresholds: Wrap(browserperformancethresholds.Service),
	// For every configured HTTP Monitor there automatically exists one of these records
	// Recommendation is to handle "golden state" via HTTP Monitors
	...
	// There automatically exists such a record for Web Applications with Session Replay turned on
	// Recommendation is to handle "golend state" via Web Application
