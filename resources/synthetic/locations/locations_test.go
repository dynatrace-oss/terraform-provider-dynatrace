...
import (
	...
	atags "github.com/dtcookie/dynatrace/api/config/synthetic/locations"
...
const ResourceName = "dynatrace_synthetic_location"
const TestDataFolder = "../../../test_data/synthetic/locations"
const RequestPath = "%s/synthetic/locations/%s"
const TestCaseID = "synthetic_locations"
...
func (test *TestStruct) URL(id string) string {
	...
	reqPath := RequestPath
	return strings.ReplaceAll(fmt.Sprintf(reqPath, envURL, id), "/api/config/v1", "/api/v1")
}
...
func (test *TestStruct) CheckDestroy(s *terraform.State) error {
	...
	restClient := atags.NewService(providerConf.DTNonConfigEnvURL, providerConf.APIToken)
