...
func (me *Condition) HandlePreconditions() error {
	if (me.EnvVar != nil) && (string(me.Item) != "UNKNOWN") {
		return fmt.Errorf("'env_var' must not be specified unless 'item' is set to 'UNKNOWN'; got 'item'='%v'", me.Item)
	}
	if (me.EnvVar == nil) && (string(me.Item) == "UNKNOWN") {
		return fmt.Errorf("'env_var' must be specified when 'item' is set to 'UNKNOWN'; got 'item'='%v'", me.Item)
	}
