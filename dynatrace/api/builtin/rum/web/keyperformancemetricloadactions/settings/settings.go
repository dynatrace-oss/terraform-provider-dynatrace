...
package keyperformancemetricloadactions
...
type Settings struct {
	FallbackThresholds *FallbackThresholds `json:"fallbackThresholds,omitempty"` // If the selected key performance metric is not detected, the **User action duration** metric is used instead.
	Kpm                LoadKpm             `json:"kpm"`                          // Key performance metric. Possible values: `CUMULATIVE_LAYOUT_SHIFT`, `DOM_INTERACTIVE`, `FIRST_INPUT_DELAY`, `LARGEST_CONTENTFUL_PAINT`, `LOAD_EVENT_END`, `LOAD_EVENT_START`, `RESPONSE_END`, `RESPONSE_START`, `SPEED_INDEX`, `USER_ACTION_DURATION`, `VISUALLY_COMPLETE`
	Scope              string              `json:"-" scope:"scope"`              // The scope of this setting (APPLICATION_METHOD, APPLICATION)
	Thresholds         *Thresholds         `json:"thresholds"`                   // Set the Tolerating and Frustrated performance thresholds for this action type.
}
