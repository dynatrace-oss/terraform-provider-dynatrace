...
func (me *service) Create(v *eventattribute.Settings) (*api.Stub, error) {
	...
	if err := req.Finish(&resp); err != nil {
		if strings.Contains(err.Error(), "builtin:span-event-attribute settings have been replaced by builtin:attribute-allow-list and builtin:attribute-masking") {
			...
			if v.Masking != eventattribute.MaskingTypes.NotMasked {
...
func (me *service) UpdateWithContext(ctx context.Context, id string, v *eventattribute.Settings) error {
	if err := me.client.Get("/api/v2/settings/schemas/builtin%3Aspan-event-attribute", 200).Finish(); err == nil {
		...
		if strings.Contains(err.Error(), "builtin:span-event-attribute settings have been replaced by builtin:attribute-allow-list and builtin:attribute-masking") {
			...
			if attributeConfig, ok := cfg.(*eventattribute.Settings); ok {
...
func (me *service) Update(id string, v *eventattribute.Settings) error {
	return errors.New("`builtin:span-event-attribute` Update function should not be called, please create a GitHub Issue")
}

func (me *service) Validate(v *eventattribute.Settings) error {
...
func (me *service) Delete(id string) error {
	return errors.New("`builtin:span-event-attribute` Delete function should not be called, please create a GitHub Issue")
}

func (me *service) GetWithContext(ctx context.Context, id string, v *eventattribute.Settings) error {
	var err error
	...
	if err = me.client.Get("/api/v2/settings/schemas/builtin%3Aspan-event-attribute", 200).Finish(); err == nil {
		...
		if strings.Contains(err.Error(), "builtin:span-event-attribute settings have been replaced by builtin:attribute-allow-list and builtin:attribute-masking") {
			...
			for _, stub := range maskingStubs {
				maskingStub := stub.Value.(*maskingsettings.Settings)
				if maskingStub.Key == stateKey {
					v.Masking = eventattribute.MaskingType(maskingStub.Masking)
					break
				}
			}
		} else {
...
func (me *service) Get(id string, v *eventattribute.Settings) error {
	return errors.New("`builtin:span-event-attribute` Get function should not be called, please create a GitHub Issue")
}
	...
	for nextPage {
		...
		if len(sol.Items) > 0 {
			for _, item := range sol.Items {
				newItem := eventattribute.Settings{}
