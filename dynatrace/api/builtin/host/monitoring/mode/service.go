...
func (me *service) List(ctx context.Context) (api.Stubs, error) {
	...
	err := me.client.Get(ctx, fullURL).Finish(listResponse)
	if err != nil {
		return stubs, err
	}
	...
	for len(nextPageKey) > 0 {
		listResponse := new(ListResponse)
		err := me.client.Get(ctx, fmt.Sprintf("/api/v2/entities?nextPageKey=%s", url.QueryEscape(nextPageKey))).Finish(listResponse)
		if err != nil {
			return stubs, err
		}
		nextPageKey = listResponse.NextPageKey
