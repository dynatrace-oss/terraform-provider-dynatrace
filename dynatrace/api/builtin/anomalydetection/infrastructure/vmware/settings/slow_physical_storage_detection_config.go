...
func (me *SlowPhysicalStorageDetectionConfig) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"custom_thresholds": {
			Type:        schema.TypeList,
			Description: "Alert if **any** condition is met in 4 out of 5 samples",
			Optional:    true, // precondition
			Elem:        &schema.Resource{Schema: new(SlowPhysicalStorageDetectionThresholds).Schema()},
			MinItems:    1,
			MaxItems:    1,
		},
		"detection_mode": {
