...
func (me *RdsHighWriteReadLatencyDetectionConfig) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"custom_thresholds": {
			Type:        schema.TypeList,
			Description: "Alert if the condition is met in 3 out of 5 samples",
			Optional:    true, // precondition
			Elem:        &schema.Resource{Schema: new(RdsHighWriteReadLatencyDetectionThresholds).Schema()},
			MinItems:    1,
			MaxItems:    1,
		},
		"detection_mode": {
...
func (me *RdsHighWriteReadLatencyDetectionConfig) HandlePreconditions() error {
	...
	if me.CustomThresholds != nil && (me.DetectionMode == nil || (me.DetectionMode != nil && string(*me.DetectionMode) != "custom")) {
		return fmt.Errorf("'custom_thresholds' must not be specified if 'detection_mode' is not set to 'custom'")
	}
