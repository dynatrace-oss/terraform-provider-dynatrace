...
resource "dynatrace_service_full_web_service" "first-instance" {
  name             = "#name#"
...
resource "dynatrace_service_full_web_service" "second-instance" {
  name             = "#name#-second"
  description      = "Created by Terraform"
  enabled          = true
  management_zones = [ dynatrace_management_zone_v2.my-mgmz.id ]
  ...
  insert_after = dynatrace_service_full_web_service.first-instance.id
}
