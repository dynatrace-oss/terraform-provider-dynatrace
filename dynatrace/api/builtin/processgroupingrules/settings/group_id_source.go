...
func (me *GroupIdSource) HandlePreconditions() error {
	if (me.AdvancedSettings != nil) && (string(me.Type) != "EXISTING") {
		return fmt.Errorf("'advanced_settings' must not be specified if 'type' is set to '%v'", me.Type)
	}
	if (me.ID == nil) && (string(me.Type) == "CUSTOM") {
		return fmt.Errorf("'id' must be specified if 'type' is set to '%v'", me.Type)
	}
	if (me.ID != nil) && (string(me.Type) != "CUSTOM") {
		return fmt.Errorf("'id' must not be specified if 'type' is set to '%v'", me.Type)
	}
	if (me.Name == nil) && (me.Property != nil && slices.Contains([]string{"ENVIRONMENT_VARIABLE", "JAVA_SYS_PROP"}, string(*me.Property))) {
		return fmt.Errorf("'name' must be specified if 'property' is set to '%v'", me.Property)
	}
	if (me.Name != nil) && (me.Property != nil && !slices.Contains([]string{"ENVIRONMENT_VARIABLE", "JAVA_SYS_PROP"}, string(*me.Property))) {
		return fmt.Errorf("'name' must not be specified if 'property' is set to '%v'", me.Property)
	}
	if (me.Property == nil) && (string(me.Type) == "EXISTING") {
		return fmt.Errorf("'property' must be specified if 'type' is set to '%v'", me.Type)
	}
	if (me.Property != nil) && (string(me.Type) != "EXISTING") {
		return fmt.Errorf("'property' must not be specified if 'type' is set to '%v'", me.Type)
	}
	return nil
}
