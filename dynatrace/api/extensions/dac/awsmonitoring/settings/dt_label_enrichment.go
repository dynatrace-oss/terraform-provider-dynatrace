...
// DTLabelEnrichment adds a Dynatrace label (`dt.*`) to all monitored AWS
// entities. Either a static `literal` value or an AWS `tag_key` lookup must
...
func (me *DTLabelEnrichment) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"tag_key": {
			Type:        schema.TypeString,
			Description: "AWS tag key whose value will be copied into the Dynatrace label. Mutually exclusive with `literal`.",
			Optional:    true,
		},
	}
}
