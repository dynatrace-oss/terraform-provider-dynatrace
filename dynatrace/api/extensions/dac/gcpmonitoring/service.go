...
// Package gcpmonitoring exposes a typed Terraform resource
// (dynatrace_gcp_monitoring_configuration) backed by the generic Extensions
...
// the extension name hard-coded to `com.dynatrace.extension.da-gcp` so the
...
import (
	...
	serviceSettings "github.com/dynatrace-oss/terraform-provider-dynatrace/dynatrace/api/extensions/dac/gcpmonitoring/settings"
...
// for ResourceTypes.GCPMonitoringConfiguration.
...
func (s *service) SchemaID() string {
	return "extensions:v2:monitoringconfigurations:" + serviceSettings.GCPExtensionName
}
...
func (s *service) Get(ctx context.Context, id string, v *serviceSettings.Settings) error {
	...
	response, err := client.GetMonitoringConfiguration(ctx, serviceSettings.GCPExtensionName, id)
	if err != nil {
		return err
	}
...
func (s *service) List(ctx context.Context) (api.Stubs, error) {
	...
	resp, err := client.ListMonitoringConfigurations(ctx, serviceSettings.GCPExtensionName)
	if err != nil {
		return nil, err
	}
...
func (s *service) Create(ctx context.Context, v *serviceSettings.Settings) (*api.Stub, error) {
	...
	response, err := client.CreateMonitoringConfiguration(ctx, serviceSettings.GCPExtensionName, body)
	if err != nil {
		return nil, err
	}
	var created struct {
...
func (s *service) Update(ctx context.Context, id string, v *serviceSettings.Settings) error {
	...
	// Intentionally NOT calling ensureExtensionVersion here — Update receives
	// a sticky state value for Optional+Computed attributes, so re-resolving
	// here would mask drift.
	body, err := json.Marshal(v)
	if err != nil {
		return err
	}
	_, err = client.UpdateMonitoringConfiguration(ctx, serviceSettings.GCPExtensionName, id, body)
	return err
}
...
func (s *service) ensureExtensionVersion(ctx context.Context, client ExtensionClient, v *serviceSettings.Settings) error {
	...
	resolved, err := resolveLatestExtensionVersion(ctx, client, serviceSettings.GCPExtensionName)
	if err != nil {
		return err
	}
...
func (s *service) Delete(ctx context.Context, id string) error {
	...
	return client.DeleteMonitoringConfiguration(ctx, serviceSettings.GCPExtensionName, id)
}
