...
// TagFilter is the shared struct for `tag_filter` and `label_filter` blocks —
// the wire schema is identical for both, GCP just keeps tags and labels in two
...
func (me *TagFilter) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"key": {
			...
			Description: "GCP tag or label key.",
			Required:    true,
		},
		"value": {
			Type:        schema.TypeString,
			Description: "GCP tag or label value to match.",
			Required:    true,
		},
		"condition": {
			Type:        schema.TypeString,
