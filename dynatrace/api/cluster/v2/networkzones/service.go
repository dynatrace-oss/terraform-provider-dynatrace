...
func (cs *ServiceClient) Create(ctx context.Context, v *networkzones.NetworkZone) (*api.Stub, error) {
	...
	if err := cs.client.Put(ctx, fmt.Sprintf("/networkZones/%s", url.PathEscape(id)), v).Finish(&response); err != nil {
		return nil, err
	}
...
func (cs *ServiceClient) Update(ctx context.Context, id string, v *networkzones.NetworkZone) error {
	return cs.client.Put(ctx, fmt.Sprintf("/networkZones/%s", id), v).Finish()
}

func (cs *ServiceClient) Delete(ctx context.Context, id string) error {
	return cs.client.Delete(ctx, fmt.Sprintf("/networkZones/%s", url.PathEscape(id))).Finish()
}

func (cs *ServiceClient) Get(ctx context.Context, id string, v *networkzones.NetworkZone) error {
	return cs.client.Get(ctx, fmt.Sprintf("/networkZones/%s", url.PathEscape(id))).Finish(v)
}
...
func (cs *ServiceClient) List(ctx context.Context) (api.Stubs, error) {
	...
	if err = cs.client.Get(ctx, "/networkZones").Finish(&stubList); err != nil {
		return nil, err
	}
	stubs := api.Stubs{}
