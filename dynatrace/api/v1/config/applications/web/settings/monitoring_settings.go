...
type MonitoringSettings struct {
	...
	IgnoreIPAddressRestrictionSettings *bool                          `json:"-"`
}
...
func (me *MonitoringSettings) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"ignore_ip_address_restriction_settings": {
			Type:        schema.TypeBool,
			Description: "Manage IP address exclusion settings with `dynatrace_web_app_ip_address_exclusion` resource",
			Optional:    true,
			Computed:    true,
		},
	}
...
func (me *MonitoringSettings) MarshalHCL(properties hcl.Properties) error {
	if err := properties.EncodeAll(map[string]any{
		...
		"ignore_ip_address_restriction_settings": me.IgnoreIPAddressRestrictionSettings,
	}); err != nil {
		return err
	}
