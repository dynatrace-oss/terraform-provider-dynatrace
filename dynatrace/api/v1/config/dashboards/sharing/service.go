...
func (me *service) Get(ctx context.Context, id string, v *sharing.DashboardSharing) error {
	...
	if err := me.client.Get(ctx, fmt.Sprintf("/api/config/v1/dashboards/%s", url.PathEscape(id))).Finish(&dbm); err != nil {
		return err
	}

	if err := me.client.Get(ctx, fmt.Sprintf("/api/config/v1/dashboards/%s/shareSettings", url.PathEscape(id))).Finish(v); err != nil {
		return err
	}
...
func (me *service) Validate(ctx context.Context, v *sharing.DashboardSharing) error {
	id := v.DashboardID
	id = strings.TrimSuffix(id, "-sharing")
	if err := me.client.Post(ctx, fmt.Sprintf("/api/config/v1/dashboards/%s/shareSettings/validator", id), v).Finish(); err != nil && !strings.HasPrefix(err.Error(), "No Content (PUT)") {
		return err
	}
	return nil
}
...
func (me *service) update(ctx context.Context, id string, v *sharing.DashboardSharing, retry int) error {
	...
	if err := me.client.Put(ctx, fmt.Sprintf("/api/config/v1/dashboards/%s/shareSettings", id), v).Finish(); err != nil && !strings.HasPrefix(err.Error(), "No Content (PUT)") {
