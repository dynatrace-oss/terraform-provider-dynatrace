...
func (me *service) Get(ctx context.Context, id string, v *order.Order) error {
	return me.client.Get(ctx, "/api/config/v1/service/requestNaming").Finish(v)
}
...
func (me *service) Create(ctx context.Context, v *order.Order) (*api.Stub, error) {
	if err := me.client.Put(ctx, "/api/config/v1/service/requestNaming/order", v).Finish(); err != nil {
		return nil, err
	}
...
func (me *service) Update(ctx context.Context, id string, v *order.Order) error {
	return me.client.Put(ctx, "/api/config/v1/service/requestNaming/order", v).Finish()
}
