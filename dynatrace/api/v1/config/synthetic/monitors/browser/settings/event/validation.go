...
type Validation struct {
	Type        ValidationType `json:"type"`              // The goal of the validation. `content_match` (check page for the specific content. Not allowed for validation inside of wait condition), `element_match` (check page for the specific element), `text_match` (check page for the specific text)
	Match       string         `json:"match"`             // The content to look for on the page. Regular expressions are allowed. In that case set `isRegex` as `true`. Required for `content_match` and `text_match`, optional for `element_match`.
	IsRegex     bool           `json:"isRegex,omitempty"` // Defines whether `match` is plain text (`false`) or a regular expression (`true`)
	...
	Target      *Target        `json:"target,omitempty"`  // The element to look for on the page
}
...
func (me *Validation) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"type": {
			...
			Description: "The goal of the validation. `content_match` (check page for the specific content. Not allowed for validation inside of wait condition), `element_match` (check page for the specific element), `text_match` (check page for the specific text).",
			Required:    true,
		},
		"match": {
			Type:        schema.TypeString,
...
func (me *Validation) HandlePreconditions() error {
	if (me.Type == ValidationTypes.ContentMatch || me.Type == ValidationTypes.TextMatch) && len(me.Match) == 0 {
		return fmt.Errorf("'match' must be a non-empty string when 'type' is '%s'", me.Type)
	}
	return nil
}

// ValidationType The goal of the validation. `content_match` (check page for the specific content. Not allowed for validation inside of wait condition), `element_match` (check page for the specific element), `text_match` (check page for the specific text)
