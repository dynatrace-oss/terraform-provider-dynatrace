...
type Settings struct {
	...
	Token *string `json:"token,omitempty"` // API access token for the Ansible Automation Controller. Please note that this token is not refreshed and can expire.
	Type  Type    `json:"type"`            // Possible Values: `Api_token`
	Url   string  `json:"url"`             // URL of the Ansible Automation Controller API endpoint. For example, https://ansible.yourdomain.com/api/v2/
}
