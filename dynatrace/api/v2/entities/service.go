...
func (me *service) Get(ctx context.Context, id string, v *entities.Settings) (err error) {
	...
	if len(me.entitySelector) > 0 {
		if err = me.client.Get(ctx, fmt.Sprintf(`/api/v2/entities?pageSize=4000%s&from=%s&entitySelector=%s&fields=tags,properties,lastSeenTms`, to, url.QueryEscape(from), url.QueryEscape(me.entitySelector))).Finish(&dataObj); err != nil {
			return err
		}
	} else {
		entitySelector := ""
		...
		if err = me.client.Get(ctx, fmt.Sprintf(`/api/v2/entities?pageSize=4000%s&from=%s&entitySelector=%s&fields=tags,properties,lastSeenTms`, to, url.QueryEscape(from), url.QueryEscape(entitySelector))).Finish(&dataObj); err != nil {
			return err
		}
	}
	...
	if dataObj.NextPageKey != nil {
		...
		for {
			...
			if err = me.client.Get(ctx, fmt.Sprintf("/api/v2/entities?nextPageKey=%s", url.PathEscape(key))).Finish(&tempObj); err != nil {
				return err
			}
