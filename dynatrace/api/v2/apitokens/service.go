/**
* @license
* Copyright 2020 Dynatrace LLC
*
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*     http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
 */

package apitokens

import (
	"context"
	"fmt"

	"github.com/dynatrace-oss/terraform-provider-dynatrace/dynatrace/api"
	"github.com/dynatrace-oss/terraform-provider-dynatrace/dynatrace/rest"
	"github.com/dynatrace-oss/terraform-provider-dynatrace/dynatrace/settings"

	apitokens "github.com/dynatrace-oss/terraform-provider-dynatrace/dynatrace/api/v2/apitokens/settings"
)

func Service(clientSet rest.ClientSet) (settings.CRUDService[*apitokens.APIToken], error) {
	return ServiceWithClient(rest.APITokenClient(clientSet))
}

func ServiceWithClient(client rest.Client) (settings.CRUDService[*apitokens.APIToken], error) {
	return &service{client}, nil
}

type service struct {
	client rest.Client
}

func (me *service) Get(ctx context.Context, id string, v *apitokens.APIToken) error {
	var err error

	req := me.client.Get(ctx, fmt.Sprintf("/api/v2/apiTokens/%s", id))
	if err = req.Finish(v); err != nil {
		return err
	}

	return nil
}

func (me *service) SchemaID() string {
	return "v2:environment:api-tokens"
}

func (me *service) List(ctx context.Context) (api.Stubs, error) {
	var err error

	req := me.client.Get(ctx, "/api/v2/apiTokens?pageSize=10000&fields=%2Bscopes%2C%2BexpirationDate%2C%2BpersonalAccessToken&sort=-creationDate")
	var tokenlist apitokens.TokenList
	if err = req.Finish(&tokenlist); err != nil {
		return nil, err
	}
	stubs := api.Stubs{}
	for _, token := range tokenlist.APITokens {
		stubs = append(stubs, &api.Stub{ID: *token.ID, Name: token.Name, Value: &token.APIToken})
	}

	return stubs, nil
}

func (me *service) Validate(v *apitokens.APIToken) error {
	return nil // no endpoint for that
}

func (me *service) Create(ctx context.Context, v *apitokens.APIToken) (*api.Stub, error) {
	var err error

	resultToken := apitokens.APIToken{}
	if err = me.client.Post(ctx, "/api/v2/apiTokens", v).Finish(&resultToken); err != nil {
		return nil, err
	}
	item := v
	if item.Enabled == nil || !*item.Enabled {
		item.Enabled = new(false)
		if err = me.client.Put(ctx, fmt.Sprintf("/api/v2/apiTokens/%s", *resultToken.ID), item).Finish(); err != nil {
			return nil, err
		}
		resultToken.Enabled = item.Enabled
	}
	resultToken.Name = item.Name
	resultToken.Scopes = item.Scopes

	return &api.Stub{ID: *resultToken.ID, Name: resultToken.Name, Value: resultToken}, nil
}

func (me *service) Update(ctx context.Context, id string, v *apitokens.APIToken) error {
	return me.client.Put(ctx, fmt.Sprintf("/api/v2/apiTokens/%s", id), v).Finish()
}

func (me *service) Delete(ctx context.Context, id string) error {
	return me.client.Delete(ctx, fmt.Sprintf("/api/v2/apiTokens/%s", id)).Finish()
}

func (me *service) New() *apitokens.APIToken {
	return new(apitokens.APIToken)
}
