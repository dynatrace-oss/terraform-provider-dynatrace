...
func EventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "events", schemaSuffix: "events"}
}

func LogsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "logs", schemaSuffix: "logs"}
}

func BusinessEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "bizevents", schemaSuffix: "events.business"}
}

func SecurityEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "events.security", schemaSuffix: "events.security"}
}

func SDLCEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "events.sdlc", schemaSuffix: "events.sdlc"}
}

func MetricsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "metrics", schemaSuffix: "metrics"}
}

func UserSessionsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "usersessions", schemaSuffix: "user.sessions"}
}

func DavisProblemsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "davis.problems", schemaSuffix: "davis.problems"}
}

func DavisEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "davis.events", schemaSuffix: "davis.events"}
}

func SystemEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "system.events", schemaSuffix: "system.events"}
}

func UserEventsService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
	return &service{credentials: credentials, kind: "user.events", schemaSuffix: "user.events"}
}

func SpansService(credentials *config.ProviderConfiguration) settings.CRUDService[*openpipeline.Configuration] {
...
func (s *service) createClient(ctx context.Context) (*caclib.Client, error) {
	platformClient, err := rest.CreatePlatformClient(ctx, s.credentials.Platform.EnvironmentURL, s.credentials.Platform)
	if err != nil {
		return nil, err
	}
