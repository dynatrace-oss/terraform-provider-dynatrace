...
// Config describes how a token is obtained. Vendor and StaticToken are mutually exclusive.
type Config struct {
	Vendor      Vendor
	Audience    string
	StaticToken string
...
// CacheKey identifies the token source belonging to this configuration. StaticToken is left out of
// it so that a credential never becomes a key in a map living as long as the process.
