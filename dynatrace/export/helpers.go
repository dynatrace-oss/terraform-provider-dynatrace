...
var HCL_NO_FORMAT = getBoolEnv("DYNATRACE_HCL_NO_FORMAT", false)

// To get more unique names when using Dynatrace Config Manager
var NAME_REPLACE_DASH = getBoolEnv("DYNATRACE_NAME_REPLACE_DASH", false)

func format(name string, force bool) {
	if HCL_NO_FORMAT {
		return
	}
	if force || getBoolEnv("DYNATRACE_FORMAT_HCL_FILES", false) {
