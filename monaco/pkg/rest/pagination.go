...
// If the response was not in the format of a paginated response, or no key was in the response, a zero value is returned.
func getPaginationValues(body []byte) (pagination Pagination) {
	json.Unmarshal(body, &pagination) // TODO: is there a need to check a potential error thrown?
	return pagination
}
...
func AddNextPageQueryParams(u *url.URL, nextPage string) *url.URL {
	...
	queryParams.Set(PARAM_NEXT_PAGE_KEY, nextPage)
